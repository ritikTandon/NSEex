...
for i in range(len(df)):
    ...
    if share in tgt_shares and symbol == 'EQ':
        ...
        b_entry = round((high + (high * 0.01)), 2)
        b_tgt = round((b_entry + (b_entry * 0.015)), 2)
        b_sl = round((b_entry - (b_entry * 0.0125)), 2)
        b_qty = 100000 // b_entry

        s_entry = round((low - (low * 0.01)), 2)
        s_tgt = round((s_entry - (s_entry * 0.015)), 2)
        s_sl = round((s_entry + (s_entry * 0.0125)), 2)
...
wb = excel.Workbooks.Open(rf"E:\Daily Data work\Trading Algorithm\{yr}\{mnth}\{date}\{date[:2]}{mnth}20{date[6:]}algo.xlsx")
...
old_wb = xl.load_workbook(rf"E:\Daily Data work\Trading Algorithm\{old_yr}\{old_mnth}\{old_date}\{old_date[:2]}{old_mnth}20{old_date[6:]}algo.xlsx")
...
today_wb = xl.load_workbook(rf"E:\Daily Data work\Trading Algorithm\{yr}\{mnth}\{date}\{date[:2]}{mnth}20{date[6:]}algo.xlsx")
...
while old_all_row < 135:       # WILL NEED TO MAKE IT DYNAMIC OR CHANGE WHEN ADDING NEW SHARES
    ...
    old_actual_tr_sheet.cell(old_actual_row, 14).value = f"""=IF(F{old_actual_row}="SELL", (E{old_actual_row}-I{old_actual_row})*'All Trades'!M{old_actual_row}, IF(F{old_actual_row}="BUY", (I{old_actual_row}-E{old_actual_row})*'All Trades'!H{old_actual_row}))"""
...
old_wb.save(rf"E:\Daily Data work\Trading Algorithm\{old_yr}\{old_mnth}\{old_date}\{old_date[:2]}{old_mnth}20{old_date[6:]}algo.xlsx")
...
excel = Dispatch('Excel.Application')
wb = excel.Workbooks.Open(rf"E:\Daily Data work\Trading Algorithm\{old_yr}\{old_mnth}\{old_date}\{old_date[:2]}{old_mnth}20{old_date[6:]}algo.xlsx")
